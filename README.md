...
## 1. 소개
* Atmega 128을 학교 수업을 계기로 배우게 되었고 수업에서 배운 것을 토대로 기말고사 프로젝트로, SNS에서 아침에 못 일어나는 아들을 위해서 아버지께서 직접 만든 모닝콜 침대인 침대가 올라오면서 아이를 침대 밖으로 나오게 함으로써 아이의 잠을 깨우는 것입니다. 이것을 모델로 삼고 블루투스와 Atmega 128을 이용한 것이며 하드웨어를 통한 프로젝트이기에 부족한 점과 아쉬운 점이 많은 작품입니다. 
...
(블루투스로 통신하는 핸드폰의 영상이 여기에는 없습니다.    2년전에 만든 작품이며 프로젝트 완성 후에  하드웨어는 학교측에 반납을 하게 되어 영상을 수정을 못하게 되었습니다.)
